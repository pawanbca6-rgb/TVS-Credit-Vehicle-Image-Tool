import streamlit as st
import time
import os
import re
import pandas as pd
import gc
import zipfile
import io
from datetime import datetime, timedelta, timezone
from bs4 import BeautifulSoup
from urllib.parse import urljoin
from PIL import Image as PILImage
from concurrent.futures import ThreadPoolExecutor, as_completed
from curl_cffi import requests as cffi_requests # <-- Ultimate TLS fingerprint spoofer

# --- STRICT SORTING ORDER CONFIGURATION ---
DESIRED_ORDER = [
    "Front Side", "Right Side", "Back Side", "Left Side", "Engine",
    "Engine & Plate Photo", "Chassis Plate", "Odometer", "Dashboard",
    "Seat", "Gear and Pedal", "Wind Shield", "All Boot", "Tyre 1",
    "Tyre 2", "Tyre 3", "Tyre 4", "Chassis Number", "Chassis Print",
    "Selfie with Vehicle", "RC Front"
]

DISALLOWED_KEYWORDS = ['score', 'qr', 'logo', 'icon', 'pdf', 'verified', 'badge', 'banner']

def sanitize_name(name):
    cleaned = re.sub(r'[^a-zA-Z0-9]', '', str(name)).lower()
    if cleaned in ['repofront', 'front', 'frontimage', 'frontview']:
        cleaned = 'frontside'
    return cleaned

ORDER_MAP = {sanitize_name(label): idx for idx, label in enumerate(DESIRED_ORDER, start=1)}

# --- STATE INITIALIZATION ---
for key in ["processing_complete", "zip_name", "total_extracted", "success_count", "failure_count", "final_df_summary"]:
    if key not in st.session_state:
        st.session_state[key] = False if key == "processing_complete" else (0 if "count" in key or "extracted" in key else (None if key == "final_df_summary" else ""))

# --- UI CONFIGURATION ---
st.set_page_config(page_title="TVS Auto-Detect Image Tool", page_icon="📸", layout="wide")

st.markdown("""
    <style>
    .metric-box { background-color: #F3F4F6; padding: 18px; border-radius: 10px; border-left: 6px solid #1E40AF; }
    .note-box { background-color: #EFF6FF; color: #1E40AF; padding: 12px; border-radius: 6px; border-left: 4px solid #2563EB; margin-bottom: 8px; font-size: 14px; font-weight: 500; }
    .premium-card { background: #FFFFFF; padding: 24px; border-radius: 14px; border-bottom: 5px solid #CBD5E1; box-shadow: 0 4px 20px rgba(0,0,0,0.03); text-align: center; }
    .card-title { font-size: 12px; color: #64748B; font-weight: 700; text-transform: uppercase; }
    .card-value { font-size: 38px; font-weight: 800; margin-top: 8px; }
    </style>
""", unsafe_allow_html=True)

st.markdown("## 📸 TVS Credit Unified Auto-Detect Tool")
st.markdown('<div class="note-box">📌 <strong>This Tool Work Only For TVS Credit links.</strong></div>', unsafe_allow_html=True)

layout_left, layout_right = st.columns([2, 1])

with layout_left:
    st.markdown("### 📂 File Upload")
    uploaded_file = st.file_uploader("Drop xlsx file here", type=["xlsx"], label_visibility="collapsed")
    min_size_kb = st.number_input("Minimum Image Size Filter (KB):", min_value=0, value=10, step=10)

valid_rows = []
total_loans_count = 0
link_columns = []

if uploaded_file is not None:
    df = pd.read_excel(uploaded_file)
    agreement_col = next((c for c in df.columns if c.upper() in ["AGREEMENT NO", "AGREEMENTNO", "LOAN NO"]), None)
    
    if not agreement_col:
        st.error("❌ Error: Sheet must contain 'AGREEMENT NO' header.")
    else:
        valid_rows = df[df[agreement_col].notna()]
        total_loans_count = len(valid_rows)
        link_columns = [col for col in df.columns if col != agreement_col and col.upper() != "AUCTION ID"]
        
        with layout_right:
            st.markdown(f"<div class='metric-box'>Total Loans Queue<br><span style='font-size:32px; font-weight:bold; color:#1E40AF;'>{total_loans_count}</span></div>", unsafe_allow_html=True)

run_engine = st.button("🚀 Start Downloading", type="primary", use_container_width=True, disabled=(uploaded_file is None))

# --- CURL_CFFI ENGINE (MIMICS CHROME EXACTLY) ---
def create_fast_session():
    # impersonate="chrome110" perfectly fakes browser signatures to bypass WAFs
    return cffi_requests.Session(impersonate="chrome110")

def download_and_verify(img_url, save_path, session, min_kb):
    try:
        r = session.get(img_url, timeout=15)
        if r.status_code == 200:
            content_length = len(r.content)
            if content_length and (content_length / 1024) < min_kb:
                return False, f"Skipped (<{min_kb}KB)"
            
            img_data = r.content
            try:
                check_img = PILImage.open(io.BytesIO(img_data))
                check_img.verify()
            except Exception:
                return False, "Corrupted Data"
            
            with open(save_path, 'wb') as f:
                f.write(img_data)
            return True, "Downloaded"
        return False, f"HTTP Error {r.status_code}"
    except Exception as e:
        return False, str(e)

def get_real_image_url(webpage_url, session):
    try:
        response = session.get(webpage_url, timeout=15)
        if response.status_code != 200:
            return None, f"HTTP {response.status_code}"
        
        soup = BeautifulSoup(response.content, 'html.parser')
        img_tag = soup.find('img', id='imgDisplay')
        if img_tag and img_tag.get('src'):
            return urljoin(webpage_url, img_tag.get('src')), "Success"
        return None, "Tag not found"
    except Exception as e:
        return None, str(e)

def extract_gallery_images_only(url, col_name, session):
    results = []
    try:
        response = session.get(url, timeout=15)
        if response.status_code != 200:
            return results, f"HTTP {response.status_code}"
        
        if 'image' in response.headers.get('Content-Type', ''):
            if not any(k in url.lower() for k in DISALLOWED_KEYWORDS):
                results.append((col_name, url))
            return results, "Direct Image"

        soup = BeautifulSoup(response.content, 'html.parser')
        gallery_section = None
        for header in soup.find_all(['h2', 'h3', 'h4', 'div']):
            if 'vehicle gallery' in header.get_text(strip=True).lower():
                gallery_section = header.find_parent(['section', 'div', 'main']) or header.parent
                break

        imgs_to_parse = gallery_section.find_all('img') if gallery_section else soup.find_all('img')
        for img in imgs_to_parse:
            src = img.get('src') or img.get('data-src')
            if not src: continue
            
            full_src = urljoin(url, src)
            alt = img.get('alt', '').strip()
            
            if any(k in f"{alt} {full_src}".lower() for k in DISALLOWED_KEYWORDS): continue

            label = alt or (img.find_parent().get_text(strip=True) if img.find_parent() else col_name)
            results.append((label, full_src))

        return results, "Success" if results else "Images rejected by filter"
    except Exception as e:
        return results, f"Error: {str(e)}"

def process_loan_auto_detect(agreement_no, row_data, columns, min_kb, base_dir):
    agreement_no = str(agreement_no).strip()
    indian_offset = timezone(timedelta(hours=5, minutes=30))
    timestamp = datetime.now(indian_offset).strftime("%d-%m-%Y %H:%M:%S")
    
    loan_folder = os.path.join(base_dir, agreement_no)
    os.makedirs(loan_folder, exist_ok=True)
    
    session = create_fast_session()
    
    success_downloads = 0
    failed_downloads = 0
    details = []
    standard_tasks = []
    gallery_raw = []
    
    for col in columns:
        url = str(row_data[col]).strip()
        if pd.isna(url) or not url.startswith('http'): continue
            
        if 'icms.tvscredit.com' in url.lower():
            img_url, err_msg = get_real_image_url(url, session)
            if img_url:
                standard_tasks.append((img_url, f"{re.sub(r'[^a-zA-Z0-9_]', '', col.replace(' ', '_'))}.jpg", col))
            else:
                failed_downloads += 1
                details.append(f"{col}: {err_msg}")
        elif 'mytvs.in' in url.lower():
            extracted, err_msg = extract_gallery_images_only(url, col, session)
            if extracted: gallery_raw.extend(extracted)
            else:
                failed_downloads += 1
                details.append(f"{col}: {err_msg}")

    for img_url, filename, col in standard_tasks:
        ok, status_str = download_and_verify(img_url, os.path.join(loan_folder, filename), session, min_kb)
        if ok: success_downloads += 1
        else: failed_downloads += 1
        details.append(f"{col}: {status_str}")

    if gallery_raw:
        slot_allocations = {}
        for label, img_url in gallery_raw:
            seq_num = ORDER_MAP.get(sanitize_name(label))
            if seq_num and seq_num not in slot_allocations:
                slot_allocations[seq_num] = (label, img_url)

        for seq_num in sorted(slot_allocations.keys()):
            filename = f"{seq_num}_{re.sub(r'[^a-zA-Z0-9_]', '', DESIRED_ORDER[seq_num - 1].replace(' ', '_'))}.jpg"
            ok, status_str = download_and_verify(slot_allocations[seq_num][1], os.path.join(loan_folder, filename), session, min_kb)
            if ok: success_downloads += 1
            else: failed_downloads += 1
            details.append(f"{filename}: {status_str}")

    return {
        "Agreement_No": agreement_no,
        "Timestamp": timestamp,
        "Total_Links_Processed": success_downloads + failed_downloads,
        "Success_Downloads": success_downloads,
        "Failed_Downloads": failed_downloads,
        "Summary_Logs": " | ".join(details)
    }

if run_engine and uploaded_file is not None:
    st.session_state.total_extracted = st.session_state.success_count = st.session_state.failure_count = 0
    CURRENT_BATCH_DIR = os.path.abspath("TVS_Downloaded_Images")
    if os.path.exists(CURRENT_BATCH_DIR):
        import shutil
        shutil.rmtree(CURRENT_BATCH_DIR)
    os.makedirs(CURRENT_BATCH_DIR, exist_ok=True)
    
    try:
        report_data = []
        engine_progressbar = st.progress(0)
        percentage_text = st.empty()  
        completed_tasks = 0
        
        agreement_col = next((c for c in df.columns if c.upper() in ["AGREEMENT NO", "AGREEMENTNO", "LOAN NO"]), None)
        
        with st.status("Spoofing TLS and Downloading Assets...", expanded=True) as log_context:
            with ThreadPoolExecutor(max_workers=min(10, len(valid_rows) if len(valid_rows)>0 else 1)) as executor:
                futures = {executor.submit(process_loan_auto_detect, row[agreement_col], row, link_columns, min_size_kb, CURRENT_BATCH_DIR): row[agreement_col] for _, row in valid_rows.iterrows()}
                
                for future in as_completed(futures):
                    res = future.result()
                    completed_tasks += 1
                    st.session_state.total_extracted += res["Success_Downloads"]
                    if res["Success_Downloads"] > 0: st.session_state.success_count += 1
                    else: st.session_state.failure_count += 1
                    report_data.append(res)
                    log_context.write(f"✅ **Done:** `{res['Agreement_No']}` -> Saved {res['Success_Downloads']} assets.")
                    
                    ratio = completed_tasks / total_loans_count
                    percentage_text.markdown(f"**📊 Progress: {int(ratio * 100)}%**")
                    engine_progressbar.progress(ratio)
                    gc.collect()
            
        if report_data:
            report_df = pd.DataFrame(report_data)
            st.session_state.final_df_summary = report_df
            report_df.to_csv(os.path.join(CURRENT_BATCH_DIR, "Batch_Download_Report.csv"), index=False)
            
            master_zip = "TVS_Master_Package.zip"
            with zipfile.ZipFile(master_zip, 'w', zipfile.ZIP_DEFLATED) as z:
                for root, dirs, files in os.walk(CURRENT_BATCH_DIR):
                    for f in files: z.write(os.path.join(root, f), os.path.relpath(os.path.join(root, f), CURRENT_BATCH_DIR))
            
            st.balloons()
            st.session_state.zip_name = master_zip
            st.session_state.processing_complete = True
            
    except Exception as e:
        st.error(f"Error: {e}")

if st.session_state.processing_complete and os.path.exists(st.session_state.zip_name):
    st.write("---")
    st.markdown("### 🏁 Dashboard Analytics")
    m1, m2, m3 = st.columns(3)
    m1.markdown(f'<div class="premium-card"><div class="card-value" style="color:#10B981;">{st.session_state.success_count}</div><span class="card-title">Success</span></div>', unsafe_allow_html=True)
    m2.markdown(f'<div class="premium-card"><div class="card-value" style="color:#EF4444;">{st.session_state.failure_count}</div><span class="card-title">Failed</span></div>', unsafe_allow_html=True)
    m3.markdown(f'<div class="premium-card"><div class="card-value" style="color:#F59E0B;">{st.session_state.total_extracted}</div><span class="card-title">Images</span></div>', unsafe_allow_html=True)
    
    st.write(" ")
    with open(st.session_state.zip_name, "rb") as fp:
        st.download_button("📥 DOWNLOAD ZIP", data=fp, file_name=f"TVS_Images_{datetime.now(timezone(timedelta(hours=5, minutes=30))).strftime('%d-%m-%Y_%H-%M-%S')}.zip", mime="application/zip", type="primary", use_container_width=True)
